...
class DataParallelPPOActor(BasePPOActor):
    ...
    def _forward_micro_batch(self, micro_batch: dict[str, torch.Tensor], temperature: float, output_attentions: bool = False) -> torch.Tensor | tuple[torch.Tensor, list[torch.Tensor]]:
        """
        Returns:
            log_probs: # (bs, response_len)
            attentions: Optional list of attention weights per layer if output_attentions=True
        """
        ...
        if output_attentions and attentions is not None:
            return log_probs, list(attentions)
        return log_probs
    ...
    @torch.no_grad()
    def compute_log_prob_with_attention(self, data: DataProto) -> tuple[torch.Tensor, list[torch.Tensor]]:
        """Compute log probability and attention weights.

        This method is used for AVAR visual attention reward computation.
        It returns both log_probs and attention weights from each layer.
        ...
        Returns:
            Tuple of (log_prob tensor, list of attention weight tensors per layer)
        """
        self.actor_module.eval()
        ...
        log_probs_lst = []
        attentions_lst = []  # List of (layer_idx, attention_weights) tuples
        if self.rank == 0:
            micro_batches = tqdm(micro_batches, desc="Compute log probs with attention", position=1)

        for micro_batch in micro_batches:
            model_inputs = {**micro_batch.batch, **micro_batch.non_tensor_batch}
            log_probs, attentions = self._forward_micro_batch(
                model_inputs, temperature=temperature, output_attentions=True
            )
            log_probs_lst.append(log_probs)
            if len(attentions_lst) == 0:
                # Initialize attentions_lst with one entry per layer
                attentions_lst = [[] for _ in range(len(attentions))]
            for layer_idx, attn in enumerate(attentions):
                attentions_lst[layer_idx].append(attn)

        log_probs = torch.concat(log_probs_lst, dim=0)

        # Concatenate attention weights across micro-batches
        all_attentions = []
        for layer_attentions in attentions_lst:
            all_attentions.append(torch.cat(layer_attentions, dim=0))

        if self.config.dynamic_batching:
            log_probs = restore_dynamic_batch(log_probs, batch_idx_list)
            all_attentions = [restore_dynamic_batch(attn, batch_idx_list) for attn in all_attentions]

        return log_probs, all_attentions
