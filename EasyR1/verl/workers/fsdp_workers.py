...
class FSDPWorker(Worker):
    ...
    @register(dispatch_mode=Dispatch.DP_COMPUTE_PROTO)
    def compute_log_probs_with_attention(self, data: DataProto):
        """Compute log probabilities and attention weights for AVAR.

        This method is used for computing visual attention reward in AVAR.
        Returns both old_log_probs and attention_weights.
        """
        assert self._has_actor
        ...
        with self.ulysses_sharding_manager:
            data = self.ulysses_sharding_manager.preprocess_data(data)
            log_probs, attentions = self.actor.compute_log_prob_with_attention(data=data)
            # Convert attention weights to CPU to save GPU memory
            attentions_cpu = [attn.cpu() for attn in attentions]
            output = DataProto.from_dict(
                tensors={"old_log_probs": log_probs},
                non_tensors={"attentions": attentions_cpu},
