...
class AlgorithmConfig:
    ...
    # AVAR Attention Guidance Parameters for RL
    use_avar_attn: bool = False
    """enable AVAR visual attention reward shaping"""
    avar_lambda_v: float = 0.3
    """visual attention reward weight (lambda_v in AVAR paper)"""
    avar_lambda_f: float = 0.1
    """format reward weight (lambda_f in AVAR paper)"""
    avar_layers: Optional[str] = None
    """layer indices for attention extraction, comma-separated (e.g., '0,1,2'). None means all layers."""
    avar_epsilon: float = 1e-6
    """epsilon for numerical stability in attention reward computation"""
