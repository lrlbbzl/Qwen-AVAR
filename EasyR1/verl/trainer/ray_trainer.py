...
from ..utils import avar_attention
...
class RayPPOTrainer:
    ...
    def _compute_avar_visual_reward(self, batch: DataProto) -> torch.Tensor:
        """Compute AVAR visual attention reward.

        Formula:
            r_visual = (total attention to image tokens) / (total attention to system tokens)

        Returns:
            Visual attention reward tensor of shape (batch_size,).
        """
        attention_weights = batch.meta_info.get("attention_weights")
        ...
        image_token_id = None
        if self.processor is not None and hasattr(self.processor, "image_token"):
            try:
        ...
        # Create token masks
        # For labels, we use the prompt part (where response_mask is 0)
        # But we need actual labels which may not be available in RL
        # So we use a heuristic based on input_ids structure
        labels = batch.batch.get("labels", input_ids)  # fallback to input_ids
        image_token_mask, system_token_mask = avar_attention.create_token_masks(
            input_ids=input_ids,
            labels=labels,
        ...
        # Parse layer indices
        layer_indices = avar_attention.parse_layer_indices(self.config.algorithm.avar_layers)

        # Compute visual attention reward
        r_visual = avar_attention.compute_visual_attention_reward(
            attention_weights=attention_weights,
            image_token_mask=image_token_mask,
            system_token_mask=system_token_mask,
            layer_indices=layer_indices,
            epsilon=self.config.algorithm.avar_epsilon,
        )

        # Scale by lambda_v
        return r_visual * self.config.algorithm.avar_lambda_v
    ...
    def fit(self):
        ...
        while self.global_step < self.training_steps:
            ...
            with timer("step", timing_raw):
                ...
                # recompute old_log_probs (with attention if AVAR is enabled)
                with timer("old", timing_raw):
                    if self.config.algorithm.use_avar_attn:
                        # Use compute_log_probs_with_attention for AVAR
                        old_log_probs_with_attn = self.actor_rollout_ref_wg.compute_log_probs_with_attention(batch)
                        batch = batch.union(old_log_probs_with_attn)
                        # Store attention weights for later use
                        if "attentions" in batch.non_tensor_batch:
                            batch.meta_info["attention_weights"] = batch.non_tensor_batch.pop("attentions")
                    else:
                ...
                with timer("adv", timing_raw):
                    ...
                    # AVAR: Add visual attention reward if enabled
                    if self.config.algorithm.use_avar_attn and "attention_weights" in batch.meta_info:
                        visual_rewards = self._compute_avar_visual_reward(batch)
                        # Add visual attention reward to token_level_rewards
                        batch.batch["token_level_rewards"] = (
                            batch.batch["token_level_rewards"] + visual_rewards.unsqueeze(-1)
                        )
                        metrics["reward/visual_attn"] = visual_rewards.mean().item()
