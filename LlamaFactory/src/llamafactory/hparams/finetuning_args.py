...
class FinetuningArguments(
    ...
    # AVAR Attention Guidance Arguments
    use_avar_attn_guide: bool = field(
        default=False,
        metadata={"help": "Whether to use AVAR attention guidance loss for SFT training."},
    )
    avar_attn_alpha: float = field(
        default=0.15,
        metadata={"help": "Alpha coefficient for image attention enhancement loss in AVAR."},
    )
    avar_attn_beta: float = field(
        default=0.15,
        metadata={"help": "Beta coefficient for system token attention suppression loss in AVAR."},
    )
    avar_attn_layers: str | None = field(
        default=None,
        metadata={
            "help": (
                "Layer indices to apply attention guidance (comma-separated). "
                "If None, applies to all layers. E.g., '0,1,2,3' for first 4 layers."
            )
        },
    )
    avar_attn_epsilon: float = field(
        default=1e-6,
        metadata={"help": "Epsilon for numerical stability in AVAR attention loss."},
    )
    ...
    def __post_init__(self):
        ...
        # Parse AVAR attention guidance layer indices
        if self.avar_attn_layers is not None:
            self.avar_attn_layer_indices: list[int] | None = [
                int(idx.strip()) for idx in self.avar_attn_layers.split(",")
            ]
        else:
            self.avar_attn_layer_indices = None
        ...
        if self.use_avar_attn_guide and self.stage != "sft":
            raise ValueError("AVAR attention guidance can only be used in SFT stage.")
